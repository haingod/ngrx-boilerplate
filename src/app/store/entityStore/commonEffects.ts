...
export class CommonEffects {
  @Effect()
  getAllAjax$ = this.actions$.pipe(
    filter(action => action.type.endsWith(GET_ALL_AJAX)),
    mergeMap(({ payload: { url }, type }: any) =>
  ...
  @Effect()
  insertAjax$ = this.actions$.pipe(
    filter(action => action.type.endsWith(INSERT_AJAX)),
    mergeMap(({ payload: { url, data }, type }: any) =>
